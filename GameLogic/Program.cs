...
namespace finalProject1
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            board[2] = new int[7] { 1, 0, 0, 0, 0, 0, 0 };
            board[3] = new int[7] { 0, 1, 0, 0, 0, 0, 0 };
            board[4] = new int[7] { 0, 0, 1, 0, 0, 0, 0 };
            board[5] = new int[7] { 0, 0, 0, 1, 0, 0, 0 };
            ...
            for (int i = 0; i < 6; i++)
            {
                for(int j = 0; j < 7; j++)
                {
                    if(j<=3)//for the horizantal either left or right from this point
                    {
                        ...
                        {
                            Console.WriteLine("player 1 Win by  horizantal");
                        }

                        //player 2
                        else if (board[i][j] == 2 && board[i][j + 1] == 2 && board[i][j + 2] == 2 && board[i][j + 3] == 2)//right side player2
                        {
                            Console.WriteLine("player 2 Win by  horizantal");
                        } 
